from _handyFunc.get_driver import *
from mainfuncSEO import getInfo
import pandas as pd
import urllib

# proxy for location
proxy = '10.141.64.176:3128'
chrome = WebDriver(proxy=proxy)

def getElement(topTitle_xpath, botTitle_xpath, url_xpath, des_xpath, page_xpath):
    print('Getting the elements')
    title_list = []
    ad_url = []
    des_list = []
    page_list = []
    counter = 0

    try:
        for tptitle in chrome.get_elements_by_xpath(topTitle_xpath):
            title_list.append(tptitle.text)
        for bttitle in chrome.get_elements_by_xpath(botTitle_xpath):
            title_list.append(bttitle.text)
    except Exception:
        print('No ad found!')
        return None
    else:
        # nL = len(name_list)
        tL = len(title_list)
        for url in chrome.get_elements_by_xpath(url_xpath):
            counter += 1
            ad_url.append(url.get_attribute('href'))
        for des in chrome.get_elements_by_xpath(des_xpath):
            des_list.append(des.text)
        for x in range(counter):
            x += 1
            page_list.append(chrome.get_element_by_xpath(page_xpath).text)

    return title_list, ad_url, des_list, page_list, tL, counter

def mainRun(kw_list):
    chrome.get_chrome()
    url = 'https://www.google.co.jp/search?q='

    #title of top/bottom ads
    topTitle_xpath = "//*[@id='tads']//*[@aria-level='3']"
    botTitle_xpath = "//*[@id='tadsb']//*[@aria-level='3']"
    #url and description
    url_xpath = "//*[@class='v5yQqb']/a"
    des_xpath = "//*[@class='MUxGbd yDYNvb lyLwlc']"
    #page number and next page button
    page_xpath = "//*[@class='YyVfkd']"
    next_xpath = "//*[@id='pnnext']/span[2]"

    keyword_main = []
    name_main = []
    title_main = []
    des_main = []
    page_main = []
    tel_main = []
    mail_main = []
    lpUrl_main = []

    for kw in kw_list:
        print(f'Target keyword: {kw}')
        query = urllib.parse.quote_plus(kw)
        chrome.driver.get(url + query)
        page = 1
        while page <= 3:
            print(f'Page: {page}')
            tries = 0
            while tries <= 4:
                tries += 1
                print(f'Try counter - {tries}')
                
                data = getElement(topTitle_xpath, botTitle_xpath, url_xpath, des_xpath, page_xpath)
                if data == None:
                    break
                elif data[4] == data[5]:
                    for x in range(data[5]):
                        keyword_main.append(kw)
                        x += 1
                    print('Add to main lists')
                    info = getInfo(chrome, data[1])

                    name_main.extend(info['name_list'])
                    tel_main.extend(info['tel_list'])
                    mail_main.extend(info['mail_list'])
                    lpUrl_main.extend(info['lpURL_list'])

                    title_main.extend(data[0])
                    des_main.extend(data[2])
                    page_main.extend(data[3])
                    break
            else: 
                pass

            if page == 3:
                break
            else:
                page += 1
                chrome.click_element_by_xpath(next_xpath)
                pass
    
    chrome.driver.close()
    
    dataPd = pd.DataFrame(
        {'keyword': keyword_main,
         'matchType': '完全一致',
         '媒体': 'Google',
         'device': 'PC',
         'name': name_main,
         'title': title_main,
         'description': des_main,
         'phoneNumber': tel_main,
         'email': mail_main,
         'url': lpUrl_main,
         'page': page_main
        }
    )
    return dataPd